    ...
    def run_concurrency_test(self, batchsize1, batchsize2, num_threads=20):
        ...
        def check_batch_size(params, config):
            ...
            if len(params) > current_batch_size:
                raise Exception(
                    f"Batch size exceeded: {len(params)} > {current_batch_size}")
        ...
        if errors:
            print(
                f"Errors with config ({batchsize1}, {batchsize2}): {errors[0]}")

        self.assertEqual(len(results), num_threads,
                         f"Failed with config ({batchsize1}, {batchsize2})")
        self.assertEqual(
            len(errors), 0, f"Errors occurred with config ({batchsize1}, {batchsize2})")
