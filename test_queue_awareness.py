...
class TestQueueAwareness(unittest.TestCase):
    def test_timeout_delayed_by_active_compute(self):
        ...
        t_order4.start()
        time.sleep(0.1)
        t_order3_first = threading.Thread(
        ...
        t_order3_second = threading.Thread(
            target=lambda: b([[0.2]], {"order": "3"}))
        ...
        # --- ASSERTIONS ---
        self.assertEqual(len(received_order3_batches), 1,
                         "Order 3 batch only be submitted once")
