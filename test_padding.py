    ...
    def test_padding_shape_mismatch(self):
        ...
        def check_input_shape(params, config):
            ...
            for i, p in enumerate(params):
                if len(p) != first_len:
                    raise ValueError(
                        f"Shape mismatch at index {i}: expected {first_len}, got {len(p)}")
        ...
        self.assertEqual(len(submitted_params), 2,
                         "Batch size should be padded to 2")
        self.assertEqual(submitted_params[0], [0.1, 0.2], "First item mismatch")
        self.assertEqual(submitted_params[1], [
                         0.1, 0.2], "Padded item mismatch (should match last input)")
